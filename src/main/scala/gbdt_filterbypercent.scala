...
object gbdt_filterbypercent {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val seeduser_size = seeduser_feature.count()
    println(sdf_time.format(new Date((System.currentTimeMillis()))) + "***********************seeduser_feature_size=" + seeduser_size + "*****************************")
    ...
    val otheruser_size = otheruser_feature.count()
    println(sdf_time.format(new Date((System.currentTimeMillis()))) + "***********************otheruser_feature_size=" + otheruser_size + "*****************************")
    ...
    if (save) {
      val user_feature_forpre = otheruser_feature.mapPartitions(iter => for (r <- iter) yield (r._1, new DenseVector(r._2)))
      val result = pre(model, user_feature_forpre, sim, topn)
      save_data(table_out, sc.parallelize(result))
    }
  ...
  def pre(model: GradientBoostedTreesModel, user_feature_forpre: RDD[(Long, DenseVector)], sim: Double, topn: Int): Array[(Long, Double)] = {
    val user_pre = user_feature_forpre.mapPartitions(iter => for (r <- iter) yield {
      val prediction = model.predict(r._2)
      (r._1, prediction)
    })
    val array = user_pre.filter(r => r._2 > sim).collect().sortWith(_._2 > _._2)
    array

  }

  def load_data_onehot(table_in: String): RDD[(Long, Array[Double], String)] = {
  ...
  def load_data(): RDD[(Long, Array[Double], String)] = {
    ...
    println("***********************load_data finished*****************************")
    df.repartition(partition_num)
    val data_rdd1 = df.mapPartitions(iter => for (r <- iter) yield {
      (r.getLong(0), r.getString(1), r.getString(2).toDouble, r.getString(3).toDouble, r.getString(4).toDouble, r.getString(5).toDouble, r.getString(6).toDouble, r.getString(7).toDouble,
    ...
    //兴趣爱好是
    println("partition_num=" + partition_num)
    data_rdd1.repartition(partition_num)
